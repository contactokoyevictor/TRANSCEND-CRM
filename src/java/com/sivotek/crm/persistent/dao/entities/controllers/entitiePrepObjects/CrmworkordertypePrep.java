 ...
 * @author okoyevictor
 */
public class CrmworkordertypePrep {
   ...
   {this.statusmessage = statusmessage;}
    ...
    public List<Response.Page.Elements.Element> crmworkordertype(List children, int publickey, int companyID)
    {
        ...
        try{
            ...
            if(company.getCompanyPK().getCompanyid() > 0)
            {
                ...
                String workordername = getElementStringValueFromList("workordername", children);
               ...
               if(companyemployee.getCompanyemployeePK().getId() > 0)
                {
                    if(!workordername.equalsIgnoreCase(""))
                    {
                        ...
                        String p = ""+bint;
                        crmworkordertypePK.setId(Integer.parseInt(p.substring(7)));
                        crmworkordertype.setCrmworkordertypePK(crmworkordertypePK);
                        crmworkordertype.setCompany(company);
                        crmworkordertype.setName(workordername);
                        crmworkordertype.setDescription(description);
                        crmworkordertype.setCompanyemployee(companyemployee);
                        crmworkordertype.setCreateddate(new Date());
                        crmworkordertype.setChangedfrom("com.sivotek.crm.persistent.dao.entities.controllers.entitiePrepObjects.CrmworkordertypePrep.class");
                        CrmworkordertypeJpaController crmworkordertypeJpaController = new CrmworkordertypeJpaController();
                        crmworkordertypeJpaController.create(crmworkordertype);
                        
                        //////////////////
                        resElement = responseOF.createResponsePageElementsElement();
                        resElement.setId("crmworkordertype");
                        resElement.setBilltypeid(crmworkordertype.getCrmworkordertypePK().getId());
                    ...
                    {
                        if(company.getCrmworkordertypeCollection().size() > 0)
                        {
                            Collection<Crmworkordertype> crmworkordertypeColl = company.getCrmworkordertypeCollection();
                            for(Crmworkordertype crmworkordertype : crmworkordertypeColl)
                            {
                                //////////////////
                                resElement = responseOF.createResponsePageElementsElement();
                                resElement.setId("crmworkordertype");
                                resElement.setBilltypeid(crmworkordertype.getCrmworkordertypePK().getId());
                                resElement.setBillname(crmworkordertype.getName());
                                resElement.setDescription(crmworkordertype.getDescription());
                                ////
                                responseElementList.add(resElement);
                            }
                        }
