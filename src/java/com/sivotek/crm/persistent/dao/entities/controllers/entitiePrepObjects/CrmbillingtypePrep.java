...
import com.sivotek.crm.persistent.dao.entities.controllers.CrmbillingtypeJpaController;
 ...
 * @author okoyevictor
 */
public class CrmbillingtypePrep {
   ...
   {this.statusmessage = statusmessage;}
    ...
    public List<Response.Page.Elements.Element> crmbillingtype(List children, int publickey, int companyID)
    {
       ...
       try{
            ...
            if(company.getCompanyPK().getCompanyid() > 0)
            {
                ...
                String billname = getElementStringValueFromList("billname", children);
                ...
                {
                    if(!billname.equalsIgnoreCase(""))
                    {
                       long bint = System.currentTimeMillis();
                       String p = ""+bint;
                       
                       CrmbillingtypePK crmbillingtypePK = new CrmbillingtypePK();
                       crmbillingtypePK.setPubkey(publickey);
                       crmbillingtypePK.setId(Integer.parseInt(p.substring(7)));
                       Crmbillingtype crmbillingtype = new Crmbillingtype();
                       crmbillingtype.setCompany(company);
                       crmbillingtype.setCrmbillingtypePK(crmbillingtypePK);
                       crmbillingtype.setBillname(billname);
                       crmbillingtype.setDescription(description);
                       crmbillingtype.setCompanyemployee(companyemployee);
                       crmbillingtype.setCreateddate(new Date());
                       crmbillingtype.setCreatedfrom("com.sivotek.crm.persistent.dao.entities.controllers.entitiePrepObjects.CrmbillintypePrep.class");
                       CrmbillingtypeJpaController crmbillingtypeJpaController = new CrmbillingtypeJpaController();
                       crmbillingtypeJpaController.create(crmbillingtype);
                       
                       //////////////////
                       resElement = responseOF.createResponsePageElementsElement();
                       resElement.setId("crmbillingtype");
                       resElement.setBilltypeid(crmbillingtype.getCrmbillingtypePK().getId());
                    ...
                    else
                    {
                        if(company.getCrmbillingtypeCollection().size() > 0)
                        {
                            ...
                            for(Crmbillingtype crmbillingtype : crmbillingtypeColl)
                            {
                                //////////////////
                                resElement = responseOF.createResponsePageElementsElement();
                                resElement.setId("crmbillingtype");
                                resElement.setBilltypeid(crmbillingtype.getCrmbillingtypePK().getId());
                                resElement.setBillname(crmbillingtype.getBillname());
                                resElement.setDescription(crmbillingtype.getDescription());
                                ////
                                responseElementList.add(resElement);
                            }
                        }
