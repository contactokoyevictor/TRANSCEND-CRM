...
import com.sivotek.crm.persistent.dao.entities.controllers.CrmexpensetypeJpaController;
 ...
 * @author okoyevictor
 */
public class CrmexpensetypePrep {
   ...
   {this.statusmessage = statusmessage;}
     ...
     public List<Response.Page.Elements.Element> crmexpensetype(List children, int publickey, int companyID)
     {
        ...
        try{
            ...
            if(company.getCompanyPK().getCompanyid() > 0)
            {
                ...
                String expensename = getElementStringValueFromList("expensename", children);
               ...
               if(companyemployee.getCompanyemployeePK().getId() > 0)
                {
                    if(!expensename.equalsIgnoreCase(""))
                    {
                        ...
                        String p = ""+bint;
                        crmexpensetypePK.setId(Integer.parseInt(p.substring(7)));
                        crmexpensetype.setCrmexpensetypePK(crmexpensetypePK);
                        
                        crmexpensetype.setCompany(company);
                        crmexpensetype.setExpensename(expensename);
                        crmexpensetype.setDescription(description);
                        crmexpensetype.setCreateddate(new Date());
                        crmexpensetype.setChangedfrom("com.sivotek.crm.persistent.dao.entities.controllers.entitiePrepObjects.Crmexpensetype.class");
                        crmexpensetype.setCompanyemployee(companyemployee);
                        CrmexpensetypeJpaController crmexpensetypeJpaController = new CrmexpensetypeJpaController();
                        crmexpensetypeJpaController.create(crmexpensetype);
                        
                        //////////////////
                        resElement = responseOF.createResponsePageElementsElement();
                        resElement.setId("crmexpensetype");
                        resElement.setExpensetypeid(crmexpensetype.getCrmexpensetypePK().getId());
                    ...
                    else
                    {
                        if(company.getCrmexpensetypeCollection().size() > 0)
                        {
                            ...
                            for(Crmexpensetype crmexpensetype : CrmexpensetypeColl)
                            {
                                //////////////////
                                resElement = responseOF.createResponsePageElementsElement();
                                resElement.setId("crmexpensetype");
                                resElement.setExpensetypeid(crmexpensetype.getCrmexpensetypePK().getId());
                                resElement.setExpensename(crmexpensetype.getExpensename());
                                resElement.setDescription(crmexpensetype.getDescription());
                                ////
                                responseElementList.add(resElement);
                            }
                        }
